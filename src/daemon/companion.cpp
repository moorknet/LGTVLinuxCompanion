...
#define			TOPOLOGY_CONFIG_FILE				"topology_config.json"
...
private:
	...
	int										event_log_callback_status_ = 0;
	...
	void												ipcCallback(std::string message, bool recursive = false);
	void												sendToIpc(int);
	...
	: prefs_(settings)
{
	...
	log_ = std::make_shared<Logging>(settings.log_level_, paths::logFile());
	ipc_server_ = std::make_shared<IpcServer>(paths::ipcSocket(),
		[this](const std::string& message, int) { ipcCallback(message); });
	if (prefs_.devices_.size() > 0)
	{
		...
		INFO(" ");
		INFO("--- LGTV Companion Service has started (v %1%) ---------------------------", APP_VERSION);
		std::string line = prefs_.getAsString();
		if (!line.empty())
		{
			std::istringstream config(line);
			while (std::getline(config, line))
				DEBUG_(CONFIG_FILE, line);
		}
		else
			DEBUG_(CONFIG_FILE, "Default configuration loaded");
		...
		if (settings.topology_support_)
		{
			...
			else
			{
				...
				std::filesystem::remove(
					prefs_.data_path_ + "/" + TOPOLOGY_CONFIG_FILE, remove_error);
			}
		}
...
std::string	 Companion::Impl::loadSavedTopologyConfiguration(void){
	std::string file = prefs_.data_path_ + "/" + TOPOLOGY_CONFIG_FILE;
...
void Companion::Impl::saveTopologyConfiguration(void){
	if (sessions_.size() == 0)
		return;
	nlohmann::json topology_json;
	std::string file = prefs_.data_path_;
	paths::ensureDir(file);
	file += "/";
...
*/
	...
	case EVENT_SYSTEM_SUSPEND:
		...
		// Buy some time during suspend, to preserve network connectivity long
		// enough to reach the TV. Upstream held the system awake with
		// SetThreadExecutionState; here the caller already holds a logind delay
		// inhibitor for the same purpose, so we only need to wait.
...
{
	// Upstream scanned the process list for a running ".scr" binary. The
	// desktop tells us directly here.
	...
	if (ioc_reset_.load() && work_was_enqueued)
	{
		...
		for (int i = 0; i < threads; i++)
		{
			thread_pool.emplace_back(
				...
				{
					// Upstream forced the thread UI language to en-US so the
					// windows event log could be parsed in a known language.
...
void Companion::Impl::sendToIpc(int dwEvent)
{
	std::string data = "";
	...
	{
	case EVENT_SYSTEM_DISPLAYOFF:
		data = "SYSTEM_DISPLAYS_OFF";
		break;
	case EVENT_SYSTEM_DISPLAYDIMMED:
		data = "SYSTEM_DISPLAYS_OFF_DIMMED";
		break;
	case EVENT_SYSTEM_DISPLAYON:
		data = "SYSTEM_DISPLAYS_ON";
		break;
	case EVENT_SYSTEM_USERBUSY:
		data = "SYSTEM_USER_BUSY";
		break;
	case EVENT_SYSTEM_USERIDLE:
		data = "SYSTEM_USER_IDLE";
		break;
	case EVENT_SYSTEM_REBOOT:
		data = "SYSTEM_REBOOT";
		break;
...
void Companion::Impl::ipcCallback(std::string message, bool recursive)
{
	if (message.length() == 0)
	{
	...
	for (auto& commandline : commands)
	{
		...
		if (command == "daemon")										
		{
			...
			// One user session per daemon instance; upstream used
			// WTSGetActiveConsoleSessionId() to find the physical console.
			std::string physical_console = currentSessionId();
		...
		else if (command == "freesyncinfo")									// SHOW FREESYNC INFORMATION
		{
			...
			ipcCallback(cmd_line_start_app_with_param, true);
			continue;
		}
		else if (command == "servicemenu")									// SHOW SERVICE MENU
		{
			...
			ipcCallback(cmd_line_service_menu, true);
			continue;
		}
		else if (command == "servicemenu_legacy_enable" || command == "servicemenu_legacy_disable")					// ENABLE FULL/LEGACY SERVICE MENU
		{
			...
			ipcCallback(cmd_line_legacy, true);
			continue;
		}
		...
		{
			std::error_code remove_error;
			std::filesystem::remove(paths::logFile(), remove_error);
			continue;
		}
		...
		else if (command == "set_curvature")										// LUNA SET CURVATURE
		{
			if (nWords > 1)
			{
				...
				else
				{
					...
					ipcCallback(cmd1, true);
					ipcCallback(cmd2, true);
					ipcCallback(cmd3, true);
					ipcCallback(cmd4, true);
				}
			}
			else
