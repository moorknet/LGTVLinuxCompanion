...
#include "paths.h"
#include "tools.h"
// MSVC pulled these in transitively via <Windows.h>; libstdc++ does not.
#include <chrono>
#include <iomanip>
#include <list>
#include <thread>
...
private:
	...
	bool makeMagicPacket(std::string, uint8_t*, unsigned char*);
	...
	, button_client_(ioc, ctx, settings, log)
{
	...
	if (device_settings_.session_key == "") // build the WebOS handshake
		webos_handshake_ = LG_HANDSHAKE_NOTPAIRED;
	else
	{
		webos_handshake_ = LG_HANDSHAKE_PAIRED;
...
void WebOsClient::Impl::workIsFinished(void)
{
	work_.clear();
	if(!workQueue_.empty())
		std::this_thread::sleep_for(std::chrono::milliseconds(20)); // potentially webOS does not like requests to be sent too quickly
	...
	{			
		for (auto& mac_str : device_settings_.mac_addresses)
		{
			if (makeMagicPacket(mac_str, buf, mac))
			{
				...
				{
					// Upstream additionally planted a transient ARP entry here so
					// the unicast packet would reach a TV that had already aged
					// out of the neighbour table. That needs CAP_NET_ADMIN on
					// linux, so it is not done; the broadcast methods cover the
					// same case without privileges.
				...
				std::string l_ip;
				if (!device_settings_.network_interface.empty())
				{
					// Pin the packet to the configured interface by binding to
					// its address. SO_BINDTODEVICE would be more direct but
					// needs CAP_NET_RAW.
					l_ip = tools::getIPforInterface(device_settings_.network_interface);
					if (l_ip == "")
						l_ip = tools::getSourceIPforDestination(device_settings_.ip);
					if (l_ip != "")
					{
...
bool WebOsClient::Impl::setSessionKey(std::string Key, std::string deviceid)
{
	if (Key.size() > 0 && deviceid.size() > 0)
	{
		...
		while (!prefslock.try_lock())
			std::this_thread::sleep_for(std::chrono::milliseconds(TIMER_LOG_MUTEX));
...
bool WebOsClient::Impl::makeMagicPacket(std::string mac_, uint8_t* buf, unsigned char* mac)
{
	if (mac_ == "")
		false;
	...
	if (mac_.length() == 12)
	{
		for (int i = 0; i < 6; ++i)
			buf[i] = 0xFF;
		for (int i = 0; i < 6; ++i)
			buf[i + 6] = static_cast<uint8_t>(std::stoul(mac_.substr(i * 2, 2), nullptr, 16));
		for (int i = 2; i <= 16; ++i)
			memcpy(&buf[i * 6], &buf[6], 6 * sizeof(uint8_t));
