...
// Empty means "the XDG default"; passing CONFIG_FILE would resolve relative to
...
int main(int argc, char* argv[])
{
	...
	// Nothing to initialise: BSD sockets need no equivalent of WSAStartup.
...
std::string CreateEvent_system(uint32_t dwEvent, std::vector<std::string> devices)
{
	EVENT event;
...
nlohmann::json SendRequest(Device device, nlohmann::json request, bool isLuna)
{
	...
	else
	{
		try
		{
			...
			//build handshake
			std::string sHandshake = LG_HANDSHAKE_PAIRED;
	...
	else
	{
		...
		while (time(0) - origtim < (Prefs.power_on_timeout_ + 1))
		{
			...
			if (execution_time >= 0 && execution_time < 1)
				std::this_thread::sleep_for(std::chrono::seconds(1 - (uint32_t)execution_time));
		}
		if(!bPoweredOn)
		{
...
void Thread_WOL(Device device)
{
	...
	while (!bTerminateThread && (time(0) - origtim < (Prefs.power_on_timeout_ + 1)))
	{
		...
		try
		{
			...
			{
				const int optval = 1;
				if (setsockopt(WOLsocket, SOL_SOCKET, SO_BROADCAST, &optval, sizeof(optval)) < 0)
					close(WOLsocket);
				else
				{
					sockaddr_in localEndpoint{};
					std::string l_ip;
					if (!device.network_interface.empty())
					{
						// Pin to the configured interface by binding its address.
						l_ip = tools::getIPforInterface(device.network_interface);
						if (l_ip == "")
							l_ip = tools::getSourceIPforDestination(device.ip);
						if (l_ip != "")
						{
